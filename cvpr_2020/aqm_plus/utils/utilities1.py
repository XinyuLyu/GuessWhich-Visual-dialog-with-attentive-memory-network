...
def loadModel(params, agent='abot', overwrite=False, quiet=False):
    ...
    if params[startArg]:
        ...
        for opt in modelOptions:
            if opt not in params:
                ...
                if continueFlag:
                    if opt not in ['aqmQStartFrom', 'aqmAStartFrom', 'saveLogs',
                                    'aqmRealQA', 'showQA', 'aqmstartFrom',
                                    'numRounds', 'expLowerLimit', 'expUpperLimit', 'selectedBatchIdxs',
                                    'lambda', 'onlyGuesser', 'numQ', 'numImg', 'runRounds',
                                    'alpha', 'qbeamSize', 'numA']:
                        if not quiet:
    ...
    if agent == 'abot':
        ...
        aEncoderParam = copy.deepcopy(encoderParam)
        aDecoderParam = copy.deepcopy(decoderParam)
        aEncoderParam['type'] = params['encoder']
        aDecoderParam['type'] = params['decoder']
        aEncoderParam['isAnswerer'] = True
        aEncoderParam['useIm'] = True
    ...
    if agent != 'AQM-qbot':
        ...
        for p in model.questioner.encoder.parameters():
            p.register_hook(clampGrad)
        for p in model.questioner.decoder.parameters():
            p.register_hook(clampGrad)
        for p in model.appAnswerer.encoder.parameters():
            p.register_hook(clampGrad)
        for p in model.appAnswerer.decoder.parameters():
            p.register_hook(clampGrad)
...
def transformer(model, seqInput, seqLens, initialState=None, returnAttns=False):
    useCUDA = torch.cuda.is_available()
    PAD = 0
    batchSize, maxLen, _ = seqInput.size()
    pos = np.zeros((batchSize, maxLen))
    cand = np.arange(1, maxLen+1)
    for idx, l in enumerate(seqLens):
        pos[idx, :l] = cand[:l]
    pos = torch.from_numpy(pos)
    if useCUDA: pos = pos.cuda()
    if initialState is None:
        return model(seqInput, pos, return_attns=returnAttns)
...
def save(model, filename, **kwargs):
    torch.save(model, filename, **kwargs)
