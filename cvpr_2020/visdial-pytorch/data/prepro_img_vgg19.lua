require 'nn'
require 'loadcaffe'
include ('/home/lvxinyu/code/visdial-pytorch/data/prepro_utils.lua')

-------------------------------------------------------------------------------
-- Input arguments and options
-------------------------------------------------------------------------------
cmd = torch.CmdLine()
cmd:text('Extract Image Features from Pretrained VGG19 Models (prototxt + caffemodel)')
cmd:text()
cmd:text('Options')
cmd:option('-inputJson', '/hhd/lvxinyu/aqm_plus/resources/data/visdial_0.9/chat_processed_params.json', 'Path to JSON file')
cmd:option('-imageRoot', '/path/to/images/', 'Path to COCO image root')
cmd:option('-cnnProto', '/hhd/lvxinyu/data/data/models/vgg19/VGG_ILSVRC_19_layers_deploy.prototxt', 'Path to the CNN prototxt')
cmd:option('-cnnModel', '/hhd/lvxinyu/data/data/models/vgg19/VGG_ILSVRC_19_layers.caffemodel', 'Path to the CNN model')
cmd:option('-trainSplit', 'train', 'Which split to use: train | trainval')
cmd:option('-batchSize', 50, 'Batch size')

cmd:option('-outName', 'visdial/data_img.h5', 'Output name')
