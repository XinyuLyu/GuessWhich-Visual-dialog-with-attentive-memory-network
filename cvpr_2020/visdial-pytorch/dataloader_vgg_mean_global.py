...
class VisDialDataset(Dataset):
    def __init__(self, params, subsets):
        ...
        if self.useIm:  # if "string"= if True
            ...
            imgFile_global = h5py.File('/hhd/lvxinyu/aqm_plus/resources/data/visdial_0.9/data_img.h5', 'r')
            imgFile = h5py.File('/home/lvxinyu/visdial/data_img.h5', 'r')
        ...
        for dtype in subsets:  # dtype: [train, val, test]
            ...
            if self.useIm:
                ...
                imgFeats = np.array(imgFile['images_' + dtype]).transpose(0, 2, 3, 1).reshape(
                    (len(imgFile['images_' + dtype]), 49, 512))
                imgFeats_global = np.array(imgFile_global['images_' + dtype])
                if not self.imgNorm:
                    continue
                print('Normalizing image features..')

                # imgFeats = normalize(imgFeats, axis=2, norm='l2')
                self.data['%s_img_fv' % dtype] = torch.FloatTensor(imgFeats)
                self.data['%s_img_fv_global' % dtype] = torch.FloatTensor(imgFeats_global)

                # self.data['%s_img_fv' % dtype] = torch.FloatTensor(imgFeats_index)
    ...
    def getIndexItem(self, dtype, idx):
        ...
        # if image needed
        if self.useIm:
            item['img_feat_mean'] = torch.mean(self.data[dtype + '_img_fv'][idx], dim =0, keepdim= False)
            item['img_feat_global'] = self.data[dtype + '_img_fv_global'][idx]
