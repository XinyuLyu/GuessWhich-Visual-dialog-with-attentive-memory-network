...
from dataloader_09 import VisDialDataset
...
for epochId, idx, batch in batch_iter(dataloader):  # 65, 2536, 20 batches of data, each iter different batch
    ...
    image = Variable(batch['img_feat'], requires_grad=False)  # (20,4096)
    ...
    for round in range(numRounds):
        ...
        if params["useCosSimilarityLoss"] or params["useHuberLoss"]:
            ...
            if params["useCosSimilarityLoss"]:
                if round > 0:
                    cos_similarity_loss += utils.cosinePenalty(cur_dialog_hidden, past_dialog_hidden[-1])
                '''
                if round > 0:
                    temp_loss = 0
                    for i in past_dialog_hidden:
                        temp_loss += utils.cosinePenalty(cur_dialog_hidden, i)
                    temp_loss /= len(past_dialog_hidden) 
                    cos_similarity_loss += temp_loss
                '''
                '''
                if round > 0:
                    temp_loss = 0
                    cos_similarity_loss1 += utils.cosinePenalty(cur_dialog_hidden, past_dialog_hidden[-1])
                    if round > 1:
                        for i in range(len(past_dialog_hidden)-1):
                            temp_loss += utils.cosinePenalty(cur_dialog_hidden, past_dialog_hidden[i])
                        temp_loss /= len(past_dialog_hidden)-1
                        cos_similarity_loss += temp_loss
                '''
            if params["useHuberLoss"]:
                if round > 0:
                    temp_loss = 0
                    huber_loss1 += utils.huberPenalty(cur_dialog_hidden, past_dialog_hidden[-1], threshold=0.1)
                    if round > 1:
                        for i in range(len(past_dialog_hidden)-1):
                            temp_loss += utils.huberPenalty(cur_dialog_hidden, past_dialog_hidden[i], threshold=0.1)
                        temp_loss /= len(past_dialog_hidden)-1
                        huber_loss += temp_loss
            if round == 0:
                if params['trainMode'] == 'sl-qbot' or params['trainMode'] == 'rl-full-QAf':
                    past_dialog_hidden.append(qBot.encoder.dialogHiddens[-1][0][0])
                elif params['trainMode'] == 'sl-abot':
                    past_dialog_hidden.append(aBot.encoder.dialogHiddens[-1][0][0])
            else:
                past_dialog_hidden.append(cur_dialog_hidden)
    ...
    loss = qBotLoss + aBotLoss + rlLoss + featLoss + cos_similarity_loss + huber_loss + 0.01*huber_loss1 + 0.01*cos_similarity_loss1
    loss.backward()
    optimizer.step()
