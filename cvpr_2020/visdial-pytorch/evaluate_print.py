...
#Community_mean_vgg:
#from dataloader_vgg_mean import VisDialDataset

#Community_mean_global_vgg:
...
if 'QBotRank' in params['evalModeList']:
    ...
    rankMetrics, roundRanks = rankQBot(qBot, dataset, split, verbose=1, exampleLimit=1400 * params['batchSize'])
    for metric, value in rankMetrics.items():
...
if 'QABotsRank' in params['evalModeList']:
    ...
    #rankMetrics, roundRanks = rankQABots(qBot, aBot, dataset, split, exampleLimit=1400 * params['batchSize'])
    for metric, value in rankMetrics.items():
...
if 'dialog' in params['evalModeList']:
    ...
    split = 'test'
    outputFolder = "/hhd/lvxinyu/visdial-pytorch/dialog_output/results"
