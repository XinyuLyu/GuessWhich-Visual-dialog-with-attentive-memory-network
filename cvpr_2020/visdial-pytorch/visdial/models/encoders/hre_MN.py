...
from visdial.models.encoders.MN import MemN2N
...
class Encoder(nn.Module):
    ...
    def concatDialogRNNInput_q(self, histIdx):
        currIns = []
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                ...
                att = self.men_n2n(torch.stack(temp_fact).permute(1,0,2), self.questionRNNStates[histIdx][0])
                self.concatDialogRNNInput(histIdx, att)
            else:
                self.concatDialogRNNInput_q(histIdx)
