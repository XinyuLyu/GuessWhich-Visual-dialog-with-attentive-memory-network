...
class Encoder(nn.Module):
    def __init__(self,
        ...
        if self.useIm == 'early':
            ...
            self.imgNet1 = nn.Linear(self.imgFeatureSize, self.imgEmbedSize)
            self.imgNet = nn.Linear(512, self.imgEmbedSize)
            self.imgEmbedDropout = nn.Dropout(0.5)
        elif self.useIm == 'late':
            quesInputSize = self.embedSize  # (300)
            self.imgNet1 = nn.Linear(self.imgFeatureSize, self.imgEmbedSize)  # (4096,300)
            self.imgNet = nn.Linear(512, self.imgEmbedSize)
            self.imgEmbedDropout = nn.Dropout(0.5)
        ...
        if self.isAnswerer:
            ...
            self.fc1 = nn.Linear(2 * self.rnnHiddenSize + 2*self.imgEmbedSize, self.rnnHiddenSize)
            self.dialogRNN = nn.LSTM(self.rnnHiddenSize, self.rnnHiddenSize, self.numLayers, batch_first=False, dropout=self.dropout)
        else:
            self.ansRNN = nn.LSTM(
                quesInputSize,
                self.rnnHiddenSize,
    ...
    def embedInputDialog(self):
        ...
        if self.isAnswerer and self.imageEmbed is None:
            self.imageEmbed = [F.tanh(self.imgNet(self.imgEmbedDropout(self.image[0]))), F.tanh(self.imgNet1(self.imgEmbedDropout(self.image[1])))] # fc (4096,300)
    ...
    def concatDialogRNNInput_a(self, histIdx, Att):
        currIns = [Att, self.questionRNNStates[histIdx][0]] #[512,512,300]
        if self.useIm == 'late':
            currIns.append(self.imageEmbed[0])
            currIns.append(self.imageEmbed[1])
    ...
    def embedDialog(self, dialogIdx):
        if self.isAnswerer:
            ...
            if dialogIdx == 0:
                ...
                self.inptInputs.append(inpt.squeeze(0))
                _, hNew = self.dialogRNN(inpt, hPrev)  # Q-Bot: rnnHiddenSize,rnnHiddenSize A-Bot: 2*rnnHiddenSize, rnnHiddenSize
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                ...
                self.concatDialogRNNInput_a(histIdx, att)
            else:
                if histIdx != 0:
                    temp_fact = []
