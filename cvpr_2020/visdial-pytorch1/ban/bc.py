...
class BCNet(nn.Module):
    ...
    def forward(self, v, q):
        if None == self.h_out:
            v_ = self.v_net(v).transpose(1,2).unsqueeze(3) #b, h_dim, v, 1
            q_ = self.q_net(q).transpose(1,2).unsqueeze(2) #b, h_dim, 1, q
        ...
        # memory efficient but slow computation
            ...
            #v_ = self.v_net(v) # b,v,d ->b, v, h_dim
            #q_ = self.q_net(q) # b,q,d ->b, q, h_dim
...
if __name__=='__main__':
    #v_dim, q_dim, h_dim, h_out
    #net = BCNet(1024,1024,1024,1024).cuda()
    net1 = BCNet(35, 34, 36, None, k=1).cuda()
    x = torch.Tensor(35,36,35).cuda() # b, v, d
    y = torch.Tensor(35,14,34).cuda() # b, q, d
    out = net1.forward(x,y)
