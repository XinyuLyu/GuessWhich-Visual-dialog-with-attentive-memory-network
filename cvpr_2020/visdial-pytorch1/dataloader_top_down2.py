    ...
    def __init__(self, params, subsets):
        ...
        for dtype in subsets:  # dtype: [train, val, test]
            ...
            if self.useIm:
                ...
                imgFeats = np.array(imgFile['images_' + dtype]).transpose(0, 2, 3, 1).reshape((len(imgFile['images_'+dtype]),49,512))
                if not self.imgNorm:
                    continue
                print('Normalizing image features..')

                #imgFeats = normalize(imgFeats, axis=2, norm='l2')  # original feature [80000, 7,7,512]
