...
def rankOptions(options, gtOptions, scores):#(20,100) (20) (20,100)
...
def rankABot(aBot, dataset, split, scoringFunction, exampleLimit=None):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        aBot.reset()
        aBot.observe(-1, image=[image, image_36], caption=caption, captionLens=captionLens)
        for round in range(numRounds):
            ...
            logProbs = aBot.evalOptions(options[:, round], optionLens[:, round], scoringFunction)# Option: forward +loss (20,100)
            logProbsCurrent = aBot.forward() # current Answer: no forward (20,22,7826)
            logProbsAll[round].append(
                scoringFunction(logProbsCurrent,
                                answers[:, round].contiguous()))#current Answer: loss (1) return scores = False
            batchRanks = rankOptions(options[:, round],
                                     correctOptionInds[:, round], logProbs) #(20,1)
    ...
    ranks = torch.cat(ranks, 0)#(25(iter)*10(rounds),20), each iter different batch
    rankMetrics = metrics.computeMetrics(ranks.cpu())
    logProbsAll = [torch.cat(lprobs, 0).mean() for lprobs in logProbsAll]#(10,25) =>(10)
    roundwiseLogProbs = torch.cat(logProbsAll, 0).data.cpu().numpy() # torch ->numpy
