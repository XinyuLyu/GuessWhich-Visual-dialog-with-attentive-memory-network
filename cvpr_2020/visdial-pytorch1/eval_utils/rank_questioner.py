...
def rankQBot(qBot, dataset, split, exampleLimit=None, verbose=0):
    ...
    numBatches = (numExamples - 1) // batchSize + 1  # how much val images per batch(384)
    ...
    roundwiseFeaturePreds = [[] for _ in range(numRounds + 1)]  # initial guess at round -1
    logProbsAll = [[] for _ in range(numRounds)]
    featLossAll = [[] for _ in range(numRounds + 1)]  # initial guess at round -1
    start_t = timer()
    for idx, batch in enumerate(
            dataloader):  # idx 0[[20,4096],[20,4096],[20,4096]...] -> idx 1[[40,4096],[40,4096],[40,4096]...]
        ...
        roundwiseFeaturePreds[0].append(predFeatures)  # batch predicted features

        for round in range(numRounds):  # predFeatures round 0[[20,4096],[],[]...] -> round 1[[20,4096],[20,4096],[]...]
            ...
            logProbsAll[round].append(utils.maskedNll(logProbsCurrent, gtQuestions[:, round].contiguous()))
    ...
    ##############################################################################
    logProbsAll = [torch.cat(lprobs, 0).mean() for lprobs in
                   logProbsAll]  # (10,384)->(10,1) mean logProbs for 384 cases at each rund
    featLossAll = [torch.cat(floss, 0).mean() for floss in
                   featLossAll]  # (11,384)->(11,1) mean featloss for 384 cases at each round
    roundwiseLogProbs = torch.cat(logProbsAll, 0).data.cpu().numpy()  # torch to numpy array
    ...
    for round in range(numRounds + 1):
        predFeatures = torch.cat(roundwiseFeaturePreds[round], 0).data.cpu().numpy()  # (384,20,4096)-> (7663,4096)
        dists = pairwise_distances(predFeatures, gtFeatures)  # cosine similarity (7663,7663)
        ranks = []
