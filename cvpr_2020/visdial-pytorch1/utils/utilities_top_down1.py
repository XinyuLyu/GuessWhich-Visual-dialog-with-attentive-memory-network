...
def loadModel(params, agent='abot', overwrite=False):
    ...
    if agent == 'abot':
        ...
        from visdial.models.answerer_top_down1 import Answerer
        ...
        from visdial.models.questioner_predict import Questioner
        model = Questioner(encoderParam, decoderParam, imgFeatureSize=encoderParam['imgFeatureSize'])
...
def dynamicRNN1(rnnModel,
    ...
    sortedSeqInput = seqInput.index_select(dim=0, index=fwdOrder) #[20,11,300]
    ...
    if initialState is not None:
        ...
        hx = None
    enc_h, (h_n, c_n) = rnnModel(packedSeqInput, hx)
    ...
    ctx = ctx.index_select(dim=0, index=bwdOrder) #[20,9,512]
    rnn_output = h_n[-1].index_select(dim=0,index=bwdOrder)
    return ctx, rnn_output, (h_n, c_n)
...
def maskedNll(seq, gtSeq, returnScores=False):
    ...
    target = torch.cat([gtSeq, padColumn], dim=1)[:, 1:seq.shape[1]+1]
