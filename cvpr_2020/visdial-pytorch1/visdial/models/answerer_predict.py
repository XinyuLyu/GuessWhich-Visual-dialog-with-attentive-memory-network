...
import visdial.models.encoders.hre_MN4 as hre_enc
...
class Answerer(Agent):
    ...
    def __init__(self, encoderParam, decoderParam, verbose=1):
        '''
            ...
            Uses an encoder networanswerk for input sequences (questions, answers and
    ...
    def observe(self, round, ans=None, caption=None, **kwargs):
        ...
        '''
        if caption is not None:  # only when round=-1, A bot with caption
            assert round == -1, "Round number should be -1 when observing" \
                                " caption, got %d instead"
            self.caption = caption
        if ans is not None:
            assert round == len(self.answers), \
                "Round number does not match number of answers observed"  # per round with one answer
    ...
    def forward(self):
        ...
        if len(self.answers) > 0:
            ...
            decIn = self.caption  # at round -1, see caption as answer
        else:
