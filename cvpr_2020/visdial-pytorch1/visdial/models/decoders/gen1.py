...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.outNet = nn.Linear(self.rnnHiddenSize, self.vocabSize)  # (512, 7826)
        self.logSoftmax = nn.LogSoftmax(dim=1)

    def forward(self, encStates, inputSeq):
        # input qt sequence(training data), output the probability of each word(7826 in total), given predicted probability & qt maximize NLL
        '''
        ...
        if inputSeq is not None:
            inputSeq = F.dropout(self.wordEmbed(inputSeq), self.dropout, training=self.training)
            #inputSeq = self.wordEmbed(inputSeq)  # input sequence (20,16/18)
            outputs, _ = self.rnn(inputSeq, encStates)  # (20,16/18,300),(H(2,20,512),C(2,20,512))
            outputs = F.dropout(outputs, self.dropout, training=self.training)
            outputSize = outputs.size()  # (20,16/18,512)
            flatOutputs = outputs.view(-1, outputSize[2])  # (320/360,512)
            flatScores = self.outNet(flatOutputs)  # fc (320/360,7826)
            flatLogProbs = self.logSoftmax(flatScores)  # (320/360,7826)
            logProbs = flatLogProbs.view(outputSize[0], outputSize[1], -1)  # (20,16/18,7826)
        return logProbs

    def forwardDecode(self,  # used in rl finetune training
                      encStates,
                      maxSeqLen=20,
                      inference='sample',
                      beamSize=1):
    ...
    def evalOptions(self, encStates, options, optionLens, scoringFunction):
        ...
        '''
        batchSize, numOptions, maxLen = options.size()  # 20, 100, 22
        optionsFlat = options.contiguous().view(-1, maxLen)  # (2000,22)

        # Reshaping H, C for each option (2,2,20,512)->(2,2,2000,512)
        encStates = [x.unsqueeze(2).repeat(1, 1, numOptions, 1).view(self.numLayers, -1, self.rnnHiddenSize) for x in encStates]
        logProbs = self.forward(encStates, inputSeq=optionsFlat) #(2000,22,7826)
        scores = scoringFunction(logProbs, optionsFlat, returnScores=True) #(2000)
