...
from visdial.models.decoders import top_down
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.imgNet = nn.Linear(4096, 2048)
        ...
        self.AttModel = top_down.AttModel()
    def forward(self, encStates, inputSeq):
        ...
        if inputSeq is not None:
            quesState, imageMean, image1_K = encStates
            imageMean = F.tanh(self.imgNet(self.imgEmbedDropout(imageMean)))
            outputs = self.AttModel(imageMean, image1_K, None, inputSeq, quesState)
        return outputs
    ...
    def evalOptions(self, encStates, options, optionLens, scoringFunction):
        ...
        quesState, imageMean, image1_K = encStates
        #imageMean = F.tanh(self.imgNet(self.imgEmbedDropout(imageMean)))
        quesState=quesState.repeat(numOptions,1)
        image1_K = image1_K.repeat(numOptions,1,1)
        imageMean = imageMean.repeat(numOptions,1)
        encStates = [quesState,imageMean,image1_K]
        logProbs = self.forward(encStates, inputSeq=optionsFlat) #(2000,21,10158)
