...
from visdial.models.encoders.MN2 import MemN2N
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.dense = nn.Linear(self.imgFeatureSize + self.rnnHiddenSize, self.rnnHiddenSize)
        ...
        if self.useIm == 'early':
            ...
            dialogInputSize = self.rnnHiddenSize + self.imgEmbedSize  # why? 2 history encoder layer
    ...
    def concatDialogRNNInput(self, histIdx, Att):
        currIns = []
        #currIns = [self.factEmbeds[histIdx][0]]  # rnn_output [20,512]
        if self.isAnswerer:
            currIns.append(Att)
            # currIns.append(self.questionRNNStates[histIdx][0])
        else:
            currIns.append(self.factEmbeds[histIdx][0])
            if histIdx != 0:
                currIns.append(self.answerRNNStates[histIdx - 1][0])
        #if not self.isAnswerer and histIdx != 0:
        #    currIns.append(self.answerRNNStates[histIdx - 1][0])
        if self.useIm == 'late':
    ...
    def embedDialog(self, dialogIdx):
        ...
        if dialogIdx != 0 and not self.isAnswerer:
            inpt = self.Dense(inpt)
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            temp_query =[self.questionRNNStates[histIdx][0],self.image]
            query = torch.cat(temp_query, -1)
            query = self.dense(query)
            att = self.men_n2n(torch.stack(temp_fact).permute(1, 0, 2), query)
