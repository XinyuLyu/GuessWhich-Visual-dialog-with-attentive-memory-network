...
from visdial.models.encoders.MN4 import MemN2N
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        if self.isAnswerer:
            ...
            self.fc1 = nn.Linear(2 * self.rnnHiddenSize + self.imgEmbedSize, 2 * self.rnnHiddenSize + self.imgEmbedSize)
        else:
            self.ansRNN = nn.LSTM(
                quesInputSize,
                self.rnnHiddenSize,
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                ...
                att = self.men_n2n(torch.stack(temp_fact).permute(1, 0, 2), ques)  # histIdx or -1 ???
