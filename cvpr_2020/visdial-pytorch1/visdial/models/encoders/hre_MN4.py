...
class Encoder(nn.Module):
    def __init__(self,
        ...
        if self.isAnswerer:
            ...
            self.fc1 = nn.Linear(2 * self.rnnHiddenSize + self.imgEmbedSize, self.rnnHiddenSize)
            ...
            self.Wq_1 = nn.Linear(self.rnnHiddenSize, self.rnnHiddenSize)
            self.Wh_1 = nn.Linear(self.rnnHiddenSize, self.rnnHiddenSize)
            self.fc1 = nn.Linear(dialogInputSize, self.rnnHiddenSize)
            self.dialogRNN = nn.LSTMCell(self.rnnHiddenSize, self.rnnHiddenSize)
        # history encoder
    ...
    def embedInputDialog(self):
        ...
        if self.isAnswerer and self.imageEmbed is None:
            self.imageEmbed = F.tanh(self.imgNet(self.imgEmbedDropout(self.image)))  # fc (4096,300)
    ...
    def embedDialog(self, dialogIdx):
        if self.isAnswerer:
            if dialogIdx == 0:
                hPrev = self.questionRNNStates[-1][1]# 2[2,20,512]
            else:
                hPrev = self.dialogHiddens[-1]
            inpt = self.dialogRNNInputs[dialogIdx] #[20,1324]
            inpt = F.tanh(self.fc1(F.dropout(inpt, self.dropout, training=True))).unsqueeze(0) #[1,20,1324]
            ...
            if dialogIdx == 0:
                ...
                inpt = F.tanh((F.dropout(inpt, self.dropout, training=True)))
                hNew = self.dialogRNN(inpt, hPrev)
                self.dialogHiddens.append(hNew)
            else:
                hPrev = self.dialogHiddens[-1]
                inpt = self.dialogRNNInputs[dialogIdx]
                inpt = F.tanh(self.fc1(F.dropout(inpt, self.dropout, training=True)))
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                ...
                att = self.men_n2n(fact, ques)  # histIdx or -1 ???
