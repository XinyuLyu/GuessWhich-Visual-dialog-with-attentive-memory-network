...
class Encoder(nn.Module):
    ...
    def embedQuestion(self, qIdx):  # find the longest sentense(chat_processed_data.h5) and pad
        ...
        logits, att = self.ban(self.imageEmbed, q)
        return logits
    ...
    def concatDialogRNNInput_a(self, histIdx, Att, bla):
    ...
    def forward(self):
        ...
        if self.isAnswerer:
            while len(self.questionRNNStates) <= round:
                ...
                bla = self.embedQuestion(qIdx)  # question Encoder
        else:
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                ...
                self.concatDialogRNNInput_a(histIdx, att, bla)
            else:
                if histIdx != 0:
                    temp_fact = []
