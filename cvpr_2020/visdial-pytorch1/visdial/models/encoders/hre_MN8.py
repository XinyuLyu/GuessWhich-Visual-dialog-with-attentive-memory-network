...
class Encoder(nn.Module):
    ...
    def concatDialogRNNInput_a(self, histIdx, img_feat_bottom):
        ...
        self.dialogRNNInputs.append(joint_repr)
        return joint_repr
    ...
    def forward(self):
        ...
        while len(self.dialogRNNInputs) <= round:
            ...
            if self.isAnswerer:
                self.feature = self.concatDialogRNNInput_a(histIdx, image_feature_bottom)
        ...
        if self.isAnswerer:
            c = self.fc_c(self.feature).unsqueeze(0)
            h = self.fc_h(self.feature).unsqueeze(0)
            dialogHidden = h, c
            return dialogHidden
