...
class Encoder(nn.Module):
    ...
    def observe(self,  # encoder take image/caption/ques/ans as memory
        ...
        if image is not None:
            assert round == -1
            self.image = image
            self.batchSize = len(self.image[0])
    ...
    def concatDialogRNNInput_a(self, histIdx, img_feat_bottom):
        q_repr = self.q_net(self.questionRNNStates[histIdx][0])
        v_repr = self.v_net(img_feat_bottom)
        joint_repr = q_repr * v_repr
        #currIns = [Att, self.questionRNNStates[histIdx][0], bla]  # [512,512,512]
        #if self.useIm == 'late':
    ...
    def forward(self):
        ...
        if self.isAnswerer:
            #self.image_vgg = F.tanh(self.imgNet(self.imgEmbedDropout(self.image[0])))
            return self.questionRNNStates[-1][1][0].squeeze(0), self.image[0], self.image[1]
