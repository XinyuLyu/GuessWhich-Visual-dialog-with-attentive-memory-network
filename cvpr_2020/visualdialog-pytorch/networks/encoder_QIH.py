...
class _netE(nn.Module):
    ...
    def forward(self, ques_emb, his_emb, img_raw, ques_hidden, his_hidden, rnd):
        img_emb = F.tanh(self.img_embed(img_raw.view(-1,1,self.img_feat_size))) #(batch,1,imgfeat)

        ques_feat, ques_hidden = self.ques_rnn(ques_emb, ques_hidden)
        ques_feat = ques_feat[-1]#(batch,nhid) [16,75,512]

        his_feat, his_hidden = self.his_rnn(his_emb, his_hidden)
        his_feat = his_feat[-1]#(batch,nhid)

        ques_emb_1 = self.Wq_1(ques_feat).view(-1, 1, self.nhid)#(batch, 1,nhid)
        his_emb_1 = self.Wh_1(his_feat).view(-1, rnd, self.nhid)#(batch, round , nhid)

        atten_emb_1 = F.tanh(his_emb_1 + ques_emb_1.expand_as(his_emb_1))#(batch, round , bhid)
        # (batch*round , nhid)  (batch*round , 1)
        ...
        his_attn_feat = torch.bmm(his_atten_weight.view(-1, 1, rnd), his_feat.view(-1, rnd, self.nhid))#(batch,1, nhid)
        spat_size = 1
        his_attn_feat = his_attn_feat.view(-1, self.nhid) #(batch, nhid)

        img_attn_feat = img_emb #(batch,1,imgfeat)

        #(batch,nhid), (batch,nhid),(batch,nhid) 1536
        concat_feat = torch.cat((ques_feat, his_attn_feat.view(-1, self.nhid), img_attn_feat.view(-1, self.nhid)),1)

        encoder_feat = F.tanh(self.fc1(F.dropout(concat_feat, self.d, training=self.training)))#[75,300]
